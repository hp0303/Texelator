...
namespace texelator_cutlass_texture_smalln {
...
// Specialized logical GEMM tile: 32 tokens x 32 output rows x K64.
// A 512x48 problem launches 16x2 = 32 one-warp CTAs.
//
// The earlier N=8 experiment is not a valid DefaultEpilogueTensorOp shape.
// With BF16 vector width 8, OutputTileOptimalThreadMap maps only lanes 0..7
// to valid rows and leaves three quarters of the 32-lane warp out of bounds.
// N=32 is the minimum output tile that maps all lanes: four 8-element column
// vectors by eight TensorOp rows. K=64 also avoids the explicitly unsupported
// two-warp-MMA-iteration MmaPipelined case.
...
class TextureIteratorB {
 public:
  ...
  using Element = texelator_cutlass_texture_smalln::Element;
...
at::Tensor texelator_cutlass_texture_smalln_bf16(
  ...
  using namespace texelator_cutlass_texture_smalln;
  TORCH_CHECK(x.scalar_type() == at::kBFloat16 && x.is_cuda() && x.is_contiguous(),
              "small-N CUTLASS texture mainloop requires contiguous CUDA BF16 input");
...
// Diagnostic control: the exact same small-N CUTLASS tile and epilogue with a
// conventional dense BF16 IteratorB. This is not a runtime backend. It exists
// only to distinguish an invalid CUTLASS tile from an invalid texture iterator.
at::Tensor texelator_cutlass_smalln_dense_bf16(
    at::Tensor weight, at::Tensor x) {
  using namespace texelator_cutlass_texture_smalln;
  using DenseKernel = typename Default::GemmKernel;
  TORCH_CHECK(weight.is_cuda() && weight.is_contiguous() &&
                  weight.scalar_type() == at::kBFloat16 && weight.dim() == 2,
              "weight must be contiguous CUDA BF16 [N,K]");
  TORCH_CHECK(x.is_cuda() && x.is_contiguous() &&
                  x.scalar_type() == at::kBFloat16 && x.dim() == 2,
              "x must be contiguous CUDA BF16 [tokens,K]");
  int rows = int(weight.size(0));
  int columns = int(weight.size(1));
  int tokens = int(x.size(0));
  TORCH_CHECK(x.size(1) == columns, "dense small-N K mismatch");
  auto output = at::empty({tokens, rows}, x.options());
  cutlass::gemm::GemmCoord problem(tokens, rows, columns);
  Swizzle swizzle;
  cutlass::gemm::GemmCoord tile(
      ThreadblockShape::kM, ThreadblockShape::kN, ThreadblockShape::kK);
  auto tiled = swizzle.get_tiled_shape(problem, tile, 1);
  dim3 grid = swizzle.get_grid_shape(tiled);
  using RefA = typename Default::Mma::IteratorA::TensorRef;
  using RefB = typename Default::Mma::IteratorB::TensorRef;
  using RefC = typename Default::Epilogue::OutputTileIterator::TensorRef;
  RefA ref_a(reinterpret_cast<Element *>(x.data_ptr()), LayoutA(columns));
  // [N,K] row-major and logical [K,N] column-major have identical storage.
  RefB ref_b(reinterpret_cast<Element *>(weight.data_ptr()), LayoutB(columns));
  RefC ref_c(reinterpret_cast<Element *>(output.data_ptr()), LayoutC(rows));
  typename OutputOp::Params output_op(1.0f, 0.0f);
  typename DenseKernel::Params params(
      problem, tiled, ref_a, ref_b, ref_c, ref_c, output_op);
  cudaStream_t stream = at::cuda::getCurrentCUDAStream(x.get_device());
  cutlass::Kernel<DenseKernel><<<
      grid, DenseKernel::kThreadCount,
      sizeof(typename DenseKernel::SharedStorage), stream>>>(params);
  CUDA_CHECK_MAINLOOP(cudaGetLastError());
  return output;
}
